...
const firRbaced = async(roleid,result) => {
    ...
    var toSend = result.map((fir)=>{return {_id:fir._id,coords:fir.Location.coordinates,type:fir.Type_of_incident, highlight:fir.Incident_Highlight,timestamp:fir.Timestamp_of_Crime, address:fir.Address, crimeCity:fir.Crime_City, penalCode:fir.Penal_code,crimeState:fir.Crime_State,
...
router.get('/fetchFIR',  (req, res) => {
    ...
    query.exec(async (error, result) => {
        ...
        if(req.cookies) {
            try{
            var role = jwt_decode(req.cookies['auth-token']).role
